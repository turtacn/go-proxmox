<div align="center">
  <!--<img src="logo.png" alt="go-proxmox Logo" width="200" height="200">-->
  
  # go-proxmox
  
  **面向虚拟机与虚拟 Kubernetes 集群的云原生统一计算平台**
  
  [![构建状态](https://img.shields.io/github/actions/workflow/status/turtacn/go-proxmox/ci.yml?branch=main&style=flat-square)](https://github.com/turtacn/go-proxmox/actions)
  [![Go 版本](https://img.shields.io/github/go-mod/go-version/turtacn/go-proxmox?style=flat-square)](https://go.dev/)
  [![许可证](https://img.shields.io/badge/license-EPL--2.0-blue?style=flat-square)](LICENSE)
  [![Go Report Card](https://goreportcard.com/badge/github.com/turtacn/go-proxmox?style=flat-square)](https://goreportcard.com/report/github.com/turtacn/go-proxmox)
  [![文档](https://img.shields.io/badge/docs-architecture-green?style=flat-square)](docs/architecture.md)
  [![欢迎 PR](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=flat-square)](CONTRIBUTING.md)
  
  [English](README.md) | [中文](README-zh.md)
  
</div>

---

## 使命宣言

**go-proxmox** 是 Proxmox VE 核心虚拟化能力的现代化 Golang 重构实现，旨在将传统虚拟机（VM）与轻量级虚拟 Kubernetes 集群（vCluster/VC）统一为单一管理平面内的一等公民计算资源。

我们保留了 Proxmox VE 久经验证的设计哲学——**直接硬件控制、极简依赖、无 libvirt 抽象层**——同时解决云原生工作负载、多租户隔离和 AI/ML 基础设施的新兴需求。

---

## 为什么选择 go-proxmox？

### 我们解决的行业痛点

| 痛点 | 传统方案 | go-proxmox 解决方案 |
|------|----------|---------------------|
| **虚拟机-容器二分法** | 独立管理平面、不一致的 API | 统一资源模型，共享存储/网络抽象 |
| **Kubernetes 复杂性** | 中小规模负载开销过重 | 原生 vCluster，内嵌 SQLite，分钟级供给 |
| **多租户隔离不足** | 仅命名空间隔离，共享控制面 | 每租户独立 apiserver，硬件级数据面隔离 |
| **AI 工作负载支持弱** | 手动 GPU 直通，无原生调度 | 一等公民 GPU/TPU 资源，AI 优化放置策略 |
| **运维负担重** | 多二进制，复杂配置 | 单二进制部署，声明式 YAML 配置 |
| **厂商锁定** | 专有 API，不透明实现 | 开源，兼容 Proxmox REST API |

### 核心差异化优势

```

┌─────────────────────────────────────────────────────────────────────────────┐
│                         go-proxmox 架构概览                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────────────┐  │
│  │   虚拟机 (VM)    │    │  虚拟集群 (VC)   │    │   AI/GPU 工作负载       │  │
│  │   (QEMU/KVM)    │    │  (vCluster)     │    │   (直通/vGPU)          │  │
│  └────────┬────────┘    └────────┬────────┘    └───────────┬─────────────┘  │
│           │                      │                         │                 │
│           └──────────────────────┼─────────────────────────┘                 │
│                                  ▼                                           │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │                      统一资源管理器                                     │  │
│  │     • 共享状态机  • 统一配额模型  • 统一事件系统                          │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│           │                      │                         │                 │
│           ▼                      ▼                         ▼                 │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────────────┐  │
│  │   存储插件       │    │   网络插件       │    │     调度引擎            │  │
│  │ ZFS/LVM/Ceph/NFS│    │ Bridge/VLAN/SDN │    │   HA/利用率/GPU         │  │
│  └─────────────────┘    └─────────────────┘    └─────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘

````

---

## 核心功能特性

### 计算资源

- **虚拟机（VM）**
  - 直接 QMP 控制 QEMU/KVM（无 libvirt）
  - 共享/本地存储的热/冷迁移
  - 完整生命周期：创建、快照、克隆、GPU 直通
  - OVMF/SeaBIOS、安全启动、vTPM 支持

- **虚拟集群（VC）**— *一等公民租户资源*
  - 每租户独立 Kubernetes 控制面
  - 分层存储：内嵌 SQLite（开发）→ 外部 etcd（生产）
  - Private Nodes 模式，kubelet-in-netns 实现完整一致性
  - 运行时隔离：L1 (runc) → L2 (Kata) → L3 (QEMU microvm)

### 统一基础设施

- **存储抽象**
  - 所有内容类型的单一插件接口
  - 后端支持：本地、ZFS、LVM-thin、Ceph RBD、NFS、iSCSI
  - VM 磁盘、VC PersistentVolume、备份、ISO 共享使用

- **网络抽象**
  - Linux Bridge 支持 VLAN/VXLAN
  - VM 网卡与 VC Pod 网络使用相同基础设施
  - 集群级 nftables 防火墙同步

### 企业级能力

- **多租户**：RBAC、配额（requests/limits/overcommit）、审计日志
- **高可用**：自动故障转移、隔离、维护模式（cordon/drain）
- **可观测性**：Prometheus 指标、OpenTelemetry 追踪、结构化日志
- **安全性**：安全启动、TPM、不可变审计日志、nftables 策略

---

## 性能基准

go-proxmox 专为生产级性能设计：

| 基准测试项 | 目标值 | 说明 |
|-----------|--------|------|
| 单节点 VM 数量 | ≥500 | 标准配置 |
| 单节点 VC 数量 | ≥1000 | 小规格虚拟集群 |
| 控制面 p99 延迟 | <100ms | API 操作 |
| VC 供给时间 | <60s | 包含控制面启动 |
| 热迁移停机时间 | <500ms | 共享存储，标准 VM |
| Oracle RAC 认证 | 计划中 | 企业数据库工作负载 |
| Kubernetes 一致性 | 100% | CNCF 认证发行版基础 |

---

## 快速开始

### 前置条件

- Linux 内核 ≥5.15 且支持 KVM
- Go 1.22+（从源码构建）
- 目标节点已安装 QEMU 8.0+

### 安装

**从二进制发布包安装：**

```bash
# 下载最新版本
curl -LO https://github.com/turtacn/go-proxmox/releases/latest/download/gpve-linux-amd64.tar.gz
tar xzf gpve-linux-amd64.tar.gz
sudo mv gpve-server gpve-agent /usr/local/bin/

# 初始化配置
gpve-server init --config /etc/gpve/config.yaml
````

**从源码安装：**

```bash
# 克隆仓库
git clone https://github.com/turtacn/go-proxmox.git
cd go-proxmox

# 构建所有二进制
make build

# 运行测试
make test

# 安装
sudo make install
```

**使用 Go Install：**

```bash
go install github.com/turtacn/go-proxmox/cmd/gpve-server@latest
go install github.com/turtacn/go-proxmox/cmd/gpve-agent@latest
go install github.com/turtacn/go-proxmox/cmd/gpvectl@latest
```

### 快速入门

**1. 启动服务器：**

```bash
# 初始化新集群
gpve-server init --cluster-name my-cluster

# 启动服务器（前台运行用于测试）
gpve-server run --config /etc/gpve/config.yaml
```

**2. 启动节点代理：**

```bash
# 在每个计算节点上执行
gpve-agent join --server https://control-plane:8443 --token <bootstrap-token>
```

**3. 创建第一个虚拟机：**

```bash
# 使用 CLI
gpvectl vm create my-vm \
  --cpu 4 \
  --memory 8Gi \
  --disk size=100Gi,storage=local-zfs \
  --network bridge=vmbr0

# 使用声明式 YAML
cat <<EOF | gpvectl apply -f -
apiVersion: gpve.io/v1
kind: VirtualMachine
metadata:
  name: my-vm
  namespace: default
spec:
  cpu:
    cores: 4
  memory:
    size: 8Gi
  disks:
    - name: root
      size: 100Gi
      storage: local-zfs
  networks:
    - name: eth0
      bridge: vmbr0
EOF
```

**4. 创建虚拟集群：**

```bash
gpvectl vc create my-cluster \
  --k8s-version 1.30 \
  --control-plane-tier standard \
  --worker-nodes 3 \
  --isolation-level L2

# 获取 kubeconfig
gpvectl vc kubeconfig my-cluster > ~/.kube/my-cluster.yaml
export KUBECONFIG=~/.kube/my-cluster.yaml
kubectl get nodes
```

---

## 使用示例

### 虚拟机生命周期管理

```go
package main

import (
    "context"
    "log"

    "github.com/turtacn/go-proxmox/pkg/client"
    "github.com/turtacn/go-proxmox/pkg/api/v1"
)

func main() {
    // 创建客户端
    c, err := client.New("https://gpve-server:8443", client.WithToken("your-token"))
    if err != nil {
        log.Fatal(err)
    }

    // 定义虚拟机规格
    vm := &v1.VirtualMachine{
        ObjectMeta: v1.ObjectMeta{
            Name:      "web-server",
            Namespace: "production",
        },
        Spec: v1.VirtualMachineSpec{
            CPU: v1.CPUSpec{
                Cores:   4,
                Sockets: 1,
                Type:    "host",
            },
            Memory: v1.MemorySpec{
                Size: "16Gi",
            },
            Disks: []v1.DiskSpec{
                {
                    Name:    "root",
                    Size:    "100Gi",
                    Storage: "ceph-pool",
                    Boot:    true,
                },
            },
            Networks: []v1.NetworkSpec{
                {
                    Name:   "eth0",
                    Bridge: "vmbr0",
                    VLAN:   100,
                },
            },
        },
    }

    // 创建虚拟机
    created, err := c.VirtualMachines("production").Create(context.Background(), vm)
    if err != nil {
        log.Fatal(err)
    }
    log.Printf("虚拟机已创建: %s (ID: %d)", created.Name, created.Status.VMID)

    // 启动虚拟机
    if err := c.VirtualMachines("production").Start(context.Background(), "web-server"); err != nil {
        log.Fatal(err)
    }

    // 热迁移到另一节点
    if err := c.VirtualMachines("production").Migrate(context.Background(), "web-server", 
        client.MigrateOptions{
            TargetNode: "node-02",
            Online:     true,
        }); err != nil {
        log.Fatal(err)
    }
}
```

### 虚拟集群操作

```go
package main

import (
    "context"
    "log"

    "github.com/turtacn/go-proxmox/pkg/client"
    "github.com/turtacn/go-proxmox/pkg/api/v1"
)

func main() {
    c, _ := client.New("https://gpve-server:8443", client.WithToken("your-token"))

    // 为租户创建虚拟集群
    vc := &v1.VirtualCluster{
        ObjectMeta: v1.ObjectMeta{
            Name:      "tenant-alpha-cluster",
            Namespace: "tenants",
            Labels: map[string]string{
                "tenant": "alpha",
                "env":    "production",
            },
        },
        Spec: v1.VirtualClusterSpec{
            KubernetesVersion: "1.30.2",
            ControlPlane: v1.ControlPlaneSpec{
                Tier:         v1.ControlPlaneTierPerformance,
                BackingStore: v1.BackingStoreEtcd,
                Replicas:     3,
            },
            WorkerNodes: v1.WorkerNodesSpec{
                Mode:  v1.WorkerModePrivate,
                Count: 5,
                Resources: v1.ResourceRequirements{
                    Requests: v1.ResourceList{
                        v1.ResourceCPU:    "2",
                        v1.ResourceMemory: "4Gi",
                    },
                    Limits: v1.ResourceList{
                        v1.ResourceCPU:    "4",
                        v1.ResourceMemory: "8Gi",
                    },
                },
            },
            IsolationLevel: v1.IsolationLevelL2, // Kata Containers
            Scheduling: v1.SchedulingSpec{
                Strategy: v1.SchedulingStrategyHA,
                AntiAffinity: &v1.AntiAffinitySpec{
                    TopologyKey: "kubernetes.io/hostname",
                },
            },
        },
    }

    created, err := c.VirtualClusters("tenants").Create(context.Background(), vc)
    if err != nil {
        log.Fatal(err)
    }

    // 等待集群就绪
    c.VirtualClusters("tenants").WaitForReady(context.Background(), created.Name)

    // 获取 kubeconfig
    kubeconfig, err := c.VirtualClusters("tenants").GetKubeconfig(context.Background(), created.Name)
    if err != nil {
        log.Fatal(err)
    }
    log.Printf("Kubeconfig:\n%s", kubeconfig)
}
```

---

## 架构概览

详细架构文档请参阅 [docs/architecture.md](docs/architecture.md)。

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              go-proxmox 架构                                     │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  ┌────────────────────────────────────────────────────────────────────────────┐ │
│  │                           展现层                                            │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │ │
│  │  │   REST API    │  │   gRPC API   │  │   CLI 工具   │  │  Web UI (*)  │   │ │
│  │  │  (Proxmox     │  │  (内部集群)   │  │  (gpvectl)   │  │  (未来)      │   │ │
│  │  │   兼容)       │  │              │  │              │  │              │   │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                          │
│  ┌────────────────────────────────────▼───────────────────────────────────────┐ │
│  │                           应用层                                            │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │ │
│  │  │  VM 服务      │  │  VC 服务     │  │  存储服务    │  │  网络服务    │   │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │ │
│  │  │  集群服务     │  │  HA 服务     │  │  任务服务    │  │  认证服务    │   │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                          │
│  ┌────────────────────────────────────▼───────────────────────────────────────┐ │
│  │                           领域层                                            │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │ │
│  │  │  VM 领域      │  │  VC 领域     │  │  存储领域    │  │  网络领域    │   │ │
│  │  │  (QMP/QEMU)  │  │  (vCluster)  │  │  (插件)      │  │ (Bridge/SDN) │   │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                     │ │
│  │  │  集群领域     │  │  调度器       │  │  状态机      │                     │ │
│  │  │  (共识)       │  │  (放置)       │  │  (生命周期)  │                     │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘                     │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
│                                       │                                          │
│  ┌────────────────────────────────────▼───────────────────────────────────────┐ │
│  │                          基础设施层                                         │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │ │
│  │  │  QMP 客户端   │  │  存储 API    │  │  网络 API    │  │  集群存储    │   │ │
│  │  │  (QEMU)      │  │ (ZFS/LVM/...) │  │  (netlink)   │  │ (etcd/CRDT)  │   │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │ │
│  │  │  指标        │  │  日志         │  │  追踪        │  │  审计日志    │   │ │
│  │  │ (Prometheus) │  │  (结构化)     │  │  (OTel)      │  │  (不可变)    │   │ │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## API 文档

完整 API 文档请参阅 [docs/apis.md](docs/apis.md)。

**API 兼容性：**

* REST API 路径兼容 Proxmox VE（VM 操作）
* 扩展路径支持 vCluster 资源
* gRPC 用于内部集群通信

**示例端点：**

```
# VM 操作（Proxmox 兼容）
GET    /api2/json/nodes/{node}/qemu
POST   /api2/json/nodes/{node}/qemu
GET    /api2/json/nodes/{node}/qemu/{vmid}/status/current
POST   /api2/json/nodes/{node}/qemu/{vmid}/status/start
POST   /api2/json/nodes/{node}/qemu/{vmid}/migrate

# VirtualCluster 操作（扩展）
GET    /api/v1/tenants/{tenantId}/vclusters
POST   /api/v1/tenants/{tenantId}/vclusters
GET    /api/v1/tenants/{tenantId}/vclusters/{name}
GET    /api/v1/tenants/{tenantId}/vclusters/{name}/kubeconfig
POST   /api/v1/tenants/{tenantId}/vclusters/{name}/upgrade
```

---

## 路线图

| 阶段      | 状态     | 描述                      |
| ------- | ------ | ----------------------- |
| Phase 0 | ✅ 完成   | 项目脚手架、核心类型、构建系统         |
| Phase 1 | 🚧 进行中 | 存储抽象、基础 VM 生命周期         |
| Phase 2 | 📋 计划中 | 网络抽象、QMP 集成             |
| Phase 3 | 📋 计划中 | 集群共识、节点管理               |
| Phase 4 | 📋 计划中 | VirtualCluster 实现       |
| Phase 5 | 📋 计划中 | HA、迁移、生产加固              |
| Phase 6 | 📋 计划中 | Web UI、高级 SDN、AI 工作负载优化 |

---

## 贡献

我们欢迎社区贡献！请在提交 PR 前阅读我们的[贡献指南](CONTRIBUTING.md)。

### 开发环境设置

```bash
# 克隆仓库
git clone https://github.com/turtacn/go-proxmox.git
cd go-proxmox

# 安装开发依赖
make deps

# 运行代码检查
make lint

# 运行所有测试
make test

# 运行集成测试（需要 QEMU）
make test-integration

# 为当前平台构建
make build
```

### 代码风格

* 遵循标准 Go 编码规范
* 提交前运行 `make lint`
* 新代码测试覆盖率 ≥80%
* 为所有导出的类型和函数编写文档

---

## 社区

* **GitHub 讨论区**: [github.com/turtacn/go-proxmox/discussions](https://github.com/turtacn/go-proxmox/discussions)
* **问题追踪**: [github.com/turtacn/go-proxmox/issues](https://github.com/turtacn/go-proxmox/issues)
* **安全问题**: [security@go-proxmox.dev](mailto:security@go-proxmox.dev)（提供 PGP 密钥）

---

## 许可证

go-proxmox 采用 [Eclipse Public License 2.0](LICENSE) 许可证。

```
Copyright (c) 2024-2025 go-proxmox 贡献者

本程序及附带材料根据 Eclipse Public License 2.0 条款提供，
该许可证可在 http://www.eclipse.org/legal/epl-2.0 获取
```

---

## 致谢

* [Proxmox VE](https://www.proxmox.com/) 提供基础虚拟化概念
* [vCluster](https://www.vcluster.com/) 提供虚拟 Kubernetes 集群技术
* [K3s](https://k3s.io/) 提供轻量级 Kubernetes 发行版
* [QEMU](https://www.qemu.org/) 提供虚拟化后端

---

<div align="center">
  <sub>由 go-proxmox 社区用 ❤️ 构建</sub>
</div>